...
...before realizing that what I was about to spend a whole bunch of time on a problem that could really use some more careful thought and a more methodical approach.
...
Since I'm only saving the data needed for the application - and thus not to be reused by another service at this point - I should refactor the function that generates the listing files to generate a flat data structure. Instead of saving listings as an array of objects, it makes more sense to treat the files like a database, and save whatever information I want for each listing as an object property, with the listing's ID as its key.
...
Hands down the fact that I started to use that first [diff library](https://www.npmjs.com/package/diff) without having more completely considered what format the data it returned was in, and also started to actually write a parser using regular expressions **before taking a moment to really reflect on what I was doing**. That's a good reminder for me to take a step back and really think through my solution before I start writing code. In my defense, it *did* work, and *did* solve the problem, and with performance that was at least in the same league as the final solution, though with a lot more code surface area.

Also, if I had spent less time on my digression, I would have liked to spend some more time planning and implementing better unit tests and especially a series of integration tests; this application would very much benefit from integration testing.
